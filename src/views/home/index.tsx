...
import type {SimpleTask as UiTask} from "@/src/views/home/components/TaskFlow";
import EmptyState from "@/src/views/home/components/EmptyState";
import ErrorAlert from "@/src/views/home/components/ErrorAlert";
import MessagesList from "@/src/views/home/components/MessagesList";
...
[{"id":101,"taskTime":"${task.taskTime ?? '2025-11-19'}","startTime":"15:20","endTime":"15:30","task":"确认技术栈","remark":"评估框架与工具","state":"pending"}]`;
    ...
    const parseTasksJson = (text: string): UiTask[] => {
        ...
        try {
            ...
            // 映射字段：title -> task，并添加 state 如果缺失
    ...
    // 点击卡片触发“二次拆解”，将返回的子任务紧贴父任务后展示
    const handleSplitTask = async (t: UiTask, ctx?: { messageId: string; taskIndex: number }) => {
        ...
        try {
            ...
            setMessages((prev) => {
                const insertAfterIdx = ctx ? prev.findIndex(m => m.id === ctx.messageId) : prev.length - 1;
                const next = [...prev];
                const newMsg = {id: String(Date.now()), role: 'assistant' as const, tasks};
                const pos = insertAfterIdx >= 0 ? insertAfterIdx + 1 : next.length;
                next.splice(pos, 0, newMsg);
                return next;
            });
        } finally {
            setLoading(false);
        ...
        <div
            ...
            <div className="relative z-10 w-full max-w-4xl mx-auto">
                ...
                <div className={`${isEmpty ? 'grid place-items-center' : ''}`}
                    ...
                    {/* 对话内容（任务卡片流） */}
                    <MessagesList messages={messages} loading={loading} onTaskClick={handleSplitTask}/>
                </div>
            </div>
