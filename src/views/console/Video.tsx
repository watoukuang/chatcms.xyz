...
export default function Video(): React.ReactElement {
  ...
  const [title, setTitle] = useState('视频生成');
  ...
  useEffect(() => {
    ...
    const meta: JobMeta = {id, title: '未命名视频任务', updatedAt: Date.now()};
    setJobs([meta]);
    setActiveId(id);
  }, []);
        ...
        <ul className="p-2 space-y-1">
          ...
          {filtered.map(j => (
            <li key={j.id} className="group">
              ...
              >
                <div className="text-sm font-medium truncate">{j.title || '未命名视频任务'}</div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="border-b border-gray-200 dark:border-white/10">
          <div className="w-full flex items-center justify-center gap-2 px-3 py-2">
            ...
            <button className="px-3 py-1.5 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700" onClick={() => { /* 生成占位 */ }}>生成视频</button>
            ...
            <div className="rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm bg-white dark:bg-[#121212]">
              <div className="px-6 py-5 space-y-4">
                <textarea
                  ...
                  placeholder="输入视频生成脚本或描述（Prompt）"
                />
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <select className="px-3 py-2 rounded-lg border border-gray-200 dark:border-white/15 bg-white dark:bg-black/20">
                    <option>时长：15秒</option>
                    <option>时长：30秒</option>
                    <option>时长：60秒</option>
                  </select>
                </div>
